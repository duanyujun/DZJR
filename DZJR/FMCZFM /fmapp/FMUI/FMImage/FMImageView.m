...
-(void)createView
{
    ...
    if(ThemeCategory==5)
    {
       // _backLayer.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4].CGColor;
        self.alpha=0.6;
    }
    else
    {
...
- (void)setAlbum:(NSArray* )imageKeyList currentIndex:(NSInteger)index
{
    ...
    for (NSString* imageKey in imageKeyList) {
        FMImage* im = [[FMImage alloc]init];
        im.url_thumb = [NSString stringWithFormat:@"%@%@",KMediaServerImagesInforURL,imageKey];
        im.url = [NSString stringWithFormat:@"%@%@",KMediaServerImagesInforURL,imageKey];
        im.isShowLarge = NO;
